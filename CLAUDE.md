...
- The Desktop tab shells out to `claudex desktop` for the Claude sessions on the project's code workspace: `list <projectId>`, `screen <windowId>`, `send <windowId> <text> --press-enter`. claudex owns the session registry, the kitty remote sockets and the i3 lookup — this app only knows window ids. The server has no DISPLAY, which is why nothing here talks to X itself. "Remote" types `/remote-control <name>` into a session that is already running, unlike the `claude-remote` route, which spawns a new headless one.
